...
precisase de um metodo que ao receber a quantidade de vendas ele retorna a comissão a ser paga pelo tipo de vendedor. 
...
const VALOR_COMISSAO_PADRAO = 1.5;
const PORCENTAGEM_BONIFICACAO_DESENVOLVEDOR = 0.1;
...
class Vendedor extends Empregado {
  ...
  comissao(quantidadeVendido) {
    switch (this.tipo) {
      case "gerente":
        this._salario += VALOR_COMISSAO_GERENTE * quantidadeVendido;
        break;
      case "vendedorUm":
        this._salario += VALOR_COMISSAO_VENDEDOR_UM * quantidadeVendido;
        break;
      case "vendedorDois":
        this._salario += VALOR_COMISSAO_VENDEDOR_DOIS * quantidadeVendido;
        break;

      default:
        this._salario += VALOR_COMISSAO_PADRAO * quantidadeVendido;
        break;
    }
  }

  getComissão(quantidadeVendido) {
    if (this.tipo === "gerente") {
      return VALOR_COMISSAO_GERENTE * quantidadeVendido;
    } else if (this.tipo === "vendedorUm") {
      return VALOR_COMISSAO_VENDEDOR_UM * quantidadeVendido;
    } else if (this.tipo === "vendedorDois") {
      return VALOR_COMISSAO_VENDEDOR_DOIS * quantidadeVendido;
    } else {
      return VALOR_COMISSAO_PADRAO * quantidadeVendido;
    }
  }
}
...
let mariaVendedora = new Vendedor(
  "Maria",
  "10/02/1880",
  "Rua tupi",
  1000,
...
console.log(
  `Salario ${mariaVendedora.getNome()} apos bonificacao: ${mariaVendedora.getSalario()}`
);
console.log(
  `Salario ${joaoVendedor.getNome()} apos bonificacao: ${joaoVendedor.getSalario()}`
);
console.log(
  `Salario ${betin.getNome()} apos bonificacao: ${betin.getSalario()}`
);

joaoVendedor.setNome("Joao da Silva");

console.log(
  `Bonificação do ${joaoVendedor.getNome()} foi de ${joaoVendedor.getBonificação()}`
);
...
console.log(
  `Salario mais comissão da vendedora ${mariaVendedora.getNome()} deu um total de: ${mariaVendedora.getSalario()}`
);
console.log(
  `Salario mais comissão da vendedor ${joaoVendedor.getNome()} deu um total de: ${joaoVendedor.getSalario()}`
);
...
console.log(
  `${mariaVendedora.getNome()} recebeu a comissão de: ${mariaVendedora.getComissão(
    2
  )}`
);
